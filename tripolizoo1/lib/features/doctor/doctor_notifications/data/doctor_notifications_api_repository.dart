...
class DoctorNotificationsApiRepository {
  ...
  Future<void> markRead(DoctorNotification notification) async {
    ...
    if (notification.type == DoctorNotificationType.healthReport) {
      await _client.post(
        '${ApiConfig.doctorNotifications}/health-reports/${notification.id}/read',
      );
      return;
    }
  ...
  DoctorNotificationType _mapType(String? value) {
    return switch (value) {
      ...
      'health_report' || 'health_report_new' => DoctorNotificationType.healthReport,
