...
class Scene {
  ...
  void _renderObject(RenderMesh renderMesh, Object o, Matrix4 model,
    ...
    if (o.mesh.texture != null && o.mesh.texcoords.length == texcoordCount) {
      ...
      final double maxTextureX = math.max(0, imageWidth - 1).toDouble();
      final double maxTextureY = math.max(0, imageHeight - 1).toDouble();
      for (int i = 0; i < texcoordCount; i++) {
        final Offset t = texcoords[i];
        // Sampling at imageWidth/imageHeight is outside the image. On a
        // closed panorama that can render as a one-pixel black seam.
        final double x = t.dx * maxTextureX + imageLeft;
        final double y = (1.0 - t.dy) * maxTextureY + imageTop;
        final int xIndex = (vertexOffset + i) * 2;
        final int yIndex = xIndex + 1;
