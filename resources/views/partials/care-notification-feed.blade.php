@foreach($careNotificationFeed as $item)
    <div class="portal-notification-item care-notification-feed-item {{ $item['is_unread'] ? 'is-unread' : 'is-read' }}"
        data-notification-kind="{{ $item['kind'] }}"
        data-reference="{{ $item['reference'] }}"
        data-unread="{{ $item['is_unread'] ? '1' : '0' }}"
        data-created-at="{{ $item['created_timestamp'] }}"
        onclick="handleCareNotificationClick(this, event)">
