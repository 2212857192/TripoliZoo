...
def process_file(f):
    ...
    # Inject filter style if .filter-select exists
    if ".filter-select" in content and "background: #f0fdf4;" not in content:
        content = re.sub(
            r'\.filter-select\s*\{[^}]+\}',
        ...
        content = re.sub(
            r'\.filter-select:focus\s*\{[^}]+\}',
            ".filter-select:focus {\n        border-color: #0d2818;\n        box-shadow: 0 0 0 3px rgba(26,74,46,0.2);\n    }",
            content
        )
        
    # Check for actual CSS rule definition rather than HTML class
    if ".modal-backdrop {" not in content and "/* ═══ MODAL ═══ */" not in content:
